...
Stage 5 在原流程中是取得 Bavaria 神秘钩索后回访的关卡。解码数据明确把 raw `27` 放在 `world1/stage02`（Bavaria Stage 3）`(24,25)` 的 foreground raw `14` 宝箱中；`world0/stage04` 没有 raw `27`。当前切片没有 Bavaria 关卡内容，因此加载 Stage 5 时至少注入工具等级 `2`，用于还原回访状态；绝不能在 Angkor Stage 5 伪造一个钩索宝箱。
...
- 进度 v6 持久化 `RelicMask`、`WorldUnlocked[3]` 与永久奖励坐标。当前只有 Angkor 会进入已完成的 `map_angkor.out`；Bavaria、Siberia 和 Shop 位置及解锁演出已实现，但其地图/商店内容仍明确显示为 Angkor 复刻范围外，不能描述成完整三世界游戏。
...
- 当前 Angkor 14 个数据关及已实现的全局封印选择外壳，与仍未实现的商店、Bavaria/Siberia 关卡内容必须分开描述，不能因为 World 0 路线通过就把整款游戏标成完成
