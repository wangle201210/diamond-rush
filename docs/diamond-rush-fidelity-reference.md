# Diamond Rush Fidelity Reference

本文保存从根 `AGENTS.md` 迁出的详细关卡、状态机、素材和验收记录。始终生效的仓库约束以根 `AGENTS.md` 为准；执行具体任务时，使用 `.agents/skills/` 中对应的 gameplay 或 asset skill，并只读取本文相关章节。
...
macOS 输入适配保持手机动作语义互斥：`Space`/`5`/数字小键盘 `5` 对应手机 `5`（交互），`Enter`/`R`/`Backspace`/数字小键盘 `*`/`Shift+8` 对应手机 `*`（回到复活点并失去一条命），`Tab` 是桌面版世界/关卡导航，教程跳过使用 `S`。不要让 Space 同时承担教程跳过，也不要让 Enter 同时承担交互、召回或世界切换。桌面版所有面向玩家的按键提示统一显示 `SPACE`、`ENTER`、`TAB`、`S`，不要直接显示手机键位 `5` 或 `*`。
...
- 身体、平台、尾击和 Angkor 徽记分别使用 `b0.f` chunk `0`、`b0.f` chunk `1`、`gen1.f` chunk `0` animation `2`、`mmv.f` chunk `3`。`DVoid()` 已把游戏区原点下移 `40px` 后才调用 `MVoid()`，因此生命条位于游戏区局部 y `5`、整屏 y `45`；它使用 `44x12` 黑底和每血 `12x8` 的 `#3bb78f` 段。Java 同样先在 `NVoid()` 绘制人物，再由 `LVoid()` 绘制巨蟒身体和三座平台，Boss 必须覆盖人物。
...
- raw `7` 下方有 raw `17` 时并非一律按竞技场门初始化为 `0x30`。Java 还要求门左右都不是 raw `26`；Bavaria Stage 5 的门 `(15,17)` 右侧是 raw `26` `(16,17)`，因此必须保留两把 foreground raw `8` 产生的低 nibble `2`，无银钥匙时保持阻挡。
...
- Java 世界地图状态 `case 15` 在人物停稳后用 `KEY_CANCEL` 进入 `bByte=28` 的 11 步 Loading，再回到全局封印选择；选择器状态 `case 27` 的 `KEY_CANCEL` 才返回主菜单。桌面版按产品要求增加独立的 `TAB` 导航：关卡内 `TAB` 回当前世界地图，地图内 `TAB` 走同一个 11 步 Loading 到世界选择；`ENTER` 不再切换世界，只在关卡内召回、在选择器中返回主菜单。地图显示 `TAB: WORLDS`，选择器显示 `ENTER: MAIN MENU`。从地图返回时箭头保留在当前世界，重新选择世界后仍读取各世界独立的解锁节点和完成进度；锁定世界和锁定节点必须继续拒绝进入。该 `TAB` 快捷路径是桌面适配，不是 Java ME 手机输入的原作声明。
...
- foreground raw `20..23`、运行时 raw `32` 消散帧和 foreground `>=80` 属于源码后置前景扫描，应在主角/动态物体之后绘制；raw `255` 是空值，绝不能按 `>=80` 转成 world frame `175`。raw `20..23` 始终直接选择 animation `raw-20`，但资源按世界不同：Angkor 在初始化时设置 gen resource bit `4`，对应 `gen0.f` chunk `4`；Bavaria 设置 bit `21`，对应 `gen2.f` chunk `1`。切换世界必须同步替换该 sprite，不能让两个世界共用 Angkor chunk 后再用 `animation^2` 调换佛像头身。
