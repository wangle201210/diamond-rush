...
- The upward branch in `aqVoid()` additionally requires the destination cell to contain packed water. At the water surface the object remains in the top wet cell and clears its accumulated fall distance. During basin drainage, the `asVoid()` target-row branch skips writing shape `7` when the next shape is `0` or `3`; this is required for Bavaria Stage 9's `(25,17)` drain to leave `(28,17)` dry before the explosive is released onto raw `37` at `(28,20)/(29,20)`.
