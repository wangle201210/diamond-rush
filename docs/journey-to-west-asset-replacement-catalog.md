...
| 闭包manifest | 13个主文件及schemas | 0 | 未开始 |
| `assets/pilgrim`新资源 | 所有可达资源可独立加载 | 0个文件 | 未开始 |
...
1. 建立`assets.json`和schemas，并从41个stage JSON生成`stage-scenes.json`、关键`10x10`视口遮罩、`asset-scene-bindings.json`及`terrain-frame-map.csv`，使每次生成都有真实坐标、相邻材质和状态上下文。
...
因此，当前文档足以指导“符合实际场景的素材重构”，不再是逐PNG换名清单；设计层面的资源逻辑已经闭合，生产和运行时尚未闭合。真正进入资产生产的下一步应先生成第2.10节的manifest、关卡视口brief及双语key表，而不是直接批量调用图像模型。
