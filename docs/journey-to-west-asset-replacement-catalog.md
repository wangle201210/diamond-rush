...
截至2026-07-13，本文档在**资源语义与闭包合同层面**已经形成可自洽的西游主题替换方案。D014的8张不透明参考板和配套结构化设计合同已经通过用户审批，审批逐图绑定SHA-256并只允许后续生产；母版仍只属于`source_archive`。首批21个B01资产ID现有73张RGBA运行候选，聚合清单追踪198个manifest、源图、Responses元数据和候选文件；这些条目仍是`candidate`且未获人工批准、未进atlas、未绑定运行时语义键。双语文本、字体包、程序化效果、其余资源和全部运行时variant尚未制作完成，不能把“文档闭环”“D014获批”或“已有候选图”表述为“素材已经完成”或“发布包已经闭环”。
...
| D014制作母版 | 角色、Boss、世界风格、功能轮廓及输出模式形成可追溯候选 | 8张PNG参考板、结构化规格、精确提示词、SHA-256来源记录及哈希绑定审批 | 已批准，仅用于后续生产与源码归档 |
| Wule最终运行资源 | 所有可达美术、音频、字体和文本可独立加载 | 21个B01资产ID已有73张运行候选、198个追踪文件；均未批准、未进atlas或运行时映射 | 进行中 |
...
因此，当前文档足以指导“符合实际场景的素材重构”，不再是逐PNG换名清单；D014母版、第一批机器清单和21项B01运行候选已经落盘，最终资源生产和Wule运行时仍未闭合。下一步继续按scene和源码切片合同生产B01/B02资源，先完成`R026/R120`专用UI-slice校验、主角和花果山最小可玩切片，再进入atlas与运行时绑定；不能无上下文批量调用图像模型。
