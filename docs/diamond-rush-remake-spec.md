...
- Freeze Hammer stores source type separately while raw `9` participates in source gravity, rolling, pushing, hook, pressure, damage, enemy crush, movement interpolation, and checkpoint restore. Frozen violet/snake frames come from `gen0.f` chunk `1` and `gen1.f` chunk `6` (world 1 pairs `gen1.f` chunk `7` enemies with frozen chunk `8`, i.java:4157-4166).
...
- Mystic Hook raw `27` is sourced from the foreground raw `14` chest at Bavaria Stage 3 `(24,25)`, not Angkor Stage 5. Since 2026-07-15 no stage entry injects tool levels or max health (Java's loader `jVoid` i.java:3258-4675 never writes `iByteArr[8]/[9]`); the only guarantees are world-entry ones (Bavaria seal entry => tool >= 1, i.java:3028-3029). Freeze-Hammer-gated content stays blocked until Siberia exists, and runtime route tests construct late-game save state themselves. Do not invent a hook chest in World 0.
